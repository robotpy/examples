...
class Drive(commands2.SubsystemBase):
    ...
    def __init__(self) -> None:
        ...
        self.leftEncoder.setDistancePerPulse(constants.DriveConstants.kEncoderDistancePerPulse)
        self.rightEncoder.setDistancePerPulse(constants.DriveConstants.kEncoderDistancePerPulse)

    def arcadeDriveCommand(self, fwd: float, rot: float) -> commands2.Command:
        # A split-stick arcade command, with forward/backward controlled by the left
        # hand, and turning controlled by the right.
        return self.run(
            lambda: self.drive.arcadeDrive(fwd, rot)
        ).setName("arcadeDrive")

    def driveDistanceCommand(self, distanceMeters: float, speed: float) -> commands2.SequentialCommandGroup:
        return self.runOnce(
            ...
            self.run(
                lambda: self.drive.arcadeDrive(speed, 0)
            )
        ).until(
            # End command when we've traveled the specified distance
            lambda: max(self.leftEncoder.getDistance(), self.rightEncoder.getDistance()) >= distanceMeters
        ).andThen(
