...
class Drivetrain:
	...
	def __init__(self) -> None:
		self.frontLeftLocation = wpimath.geometry.Transform2d(0.381, 0.381)
		self.frontRightLocation = wpimath.geometry.Transform2d(0.381, -0.381)
		self.backLeftLocation = wpimath.geometry.Transform2d(-0.381, 0.381)
		self.backRightLocaton = wpimath.geometry.Transform2d(-0.381, -0.381)
	...
	def drive(self, xSpeed: float, ySpeed: float, rot: float, fieldRelative: bool, periodSeconds: float) -> None:
		...
		swerveModuleStates = self.kinematics.toSwerveModuleStates(
			wpimath.kinematics.ChassisSpeeds.discretize(
				wpilib.ChassisSpeeds.fromFieldRelativeSpeeds(
					xSpeed, ySpeed, rot, self.gyro.getRotation2d()
				) if fieldRelative else wpilib.ChassisSpeeds(xSpeed, ySpeed, rot),
				periodSeconds
			)
		)
		wpilib.kinematics.SwerveDriveKinematics.desaturateWheelSpeeds(swerveModuleStates, kMaxSpeed)
